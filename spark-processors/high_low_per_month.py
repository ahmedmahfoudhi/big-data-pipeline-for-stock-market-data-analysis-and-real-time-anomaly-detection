...
cassandra_df = spark.read \
    ...
    .load()
high_low_per_month_df = cassandra_df.groupBy("month").agg(F.sum("high").alias("total_high"), F.sum("low").alias("total_low"))
high_low_per_month_collected_df = high_low_per_month_df.agg(F.collect_list(F.struct("month", "total_high", "total_low")).alias("value"))
high_low_per_month_with_id = high_low_per_month_collected_df.withColumn("id", F.lit("high_low_per_month")).withColumn("created_at", F.lit(F.current_timestamp()))
