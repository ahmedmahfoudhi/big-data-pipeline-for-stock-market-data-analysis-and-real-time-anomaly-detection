...
def write_to_mongo(data_df, id):
    ...
    data_df_with_additional_columns = data_df.withColumn("id", F.lit(id)).withColumn("created_at", F.current_timestamp())
    data_df_with_additional_columns.write.format("mongo").mode("append").save()

nb_of_entries = cassandra_df.count()
nb_of_entries_df = spark.createDataFrame([(nb_of_entries,)], ["value"]).withColumn("value", F.lit(nb_of_entries))
